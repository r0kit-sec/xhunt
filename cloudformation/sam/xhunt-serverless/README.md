...
```bash
job-dispatcher$ sam build --use-container
```
...
```bash
job-dispatcher$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
job-dispatcher$ sam logs -n HelloWorldFunction --stack-name job-dispatcher --tail
```
...
```bash
job-dispatcher$ pip install -r tests/requirements.txt --user
# unit test
job-dispatcher$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
job-dispatcher$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name job-dispatcher
```
