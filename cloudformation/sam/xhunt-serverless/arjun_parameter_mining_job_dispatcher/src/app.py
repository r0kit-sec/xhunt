...
def submit_jobs(messages):
    ...
    job_definition_arn = os.environ['TASK_CLUSTER_ARJUN_JOB_DEFINITION_ARN']
    ...
    for message in messages:
        try:
            raw_command = f"./tasks/arjun_single_url.sh {message['body']} {dalfox_queue}"
            command = shlex.split(raw_command)
            logging.info(f"Submitting command: {raw_command}")
            
            response = client.submit_job(
                jobName=f"arjun-{message['messageId']}",
